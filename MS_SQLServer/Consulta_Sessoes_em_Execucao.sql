SELECT  
  ...
  t1.status AS [Status],
  '(' + CAST(waittime AS VARCHAR(15)) + 'ms) ' + lastwaittype AS LastWaitType,
  (
  ...
  GETDATE() AS ScanDate,
  CASE
    WHEN t1.last_batch > sqlserver_start_time
     THEN
      DATEDIFF_BIG(SECOND, t1.last_batch, GETDATE())
    ELSE
      0
  END AS ExecutionTimeSec,
  t1.open_tran AS OpenTran,
...
WHERE (
          ...
          AND
          (
              t1.status IN ( 'runnable', 'suspended', 'rollback' )
              OR t1.blocked <> 0
          )
      );
